...
def train(args):
    ...
    class IGAT_GNN_Dataset(torch.utils.data.Dataset):
    ...
    train_dataset = IGAT_GNN_Dataset(x_train)
    validation_dataset = IGAT_GNN_Dataset(x_validation)
    test_dataset = IGAT_GNN_Dataset(x_test)
    ...
    net = IGAT_GNN(num_user, num_entity, num_relation, kg, args, device, UAndI).to(device)
