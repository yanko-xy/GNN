...
from conv import myGATConv, DropLearner, UserEnhance
...
class myGAT(nn.Module):
    def __init__(self, args, num_entity, num_etypes, num_hidden, num_classes, num_layers,
        ...
        self.user_enhance = UserEnhance(self.cfe_size + 112, self.cfe_size + 112, feat_drop, device=self.device)
    ...
    def calc_cf_loss(self, g, sub_g, kg, user_id, pos_item, neg_item):#(self, g, user_id, item_id, pos_mat):
        ...
        user_neighbors = []
        for uid in user_id:
            neighbors = g.predecessors(uid).tolist() + g.successors(uid).tolist()
            user_neighbors.extend(neighbors)
        # 合并所有需要的节点并去重
        all_nodes = torch.tensor(list(set(user_neighbors + batch_nodes))).to(self.device)
        batch_graph = dgl.node_subgraph(g, all_nodes)
        sub_emb = embedding[all_nodes]


        p_emb = embedding[pos_item] # [batch_size, n_feats + num_layers * num_heads * num_hidden + num_classes + pretrained_embed_size]
        up_emb = self.user_enhance(batch_graph, sub_emb, user_id, u_emb, p_emb)
        n_emb = embedding[neg_item] # [batch_size, n_feats + num_layers * num_heads * num_hidden + num_classes + pretrained_embed_size]
        un_emb = self.user_enhance(batch_graph, sub_emb, user_id, u_emb, n_emb)
        pos_scores = (up_emb * p_emb).sum(dim=1) # [batch_size]
        neg_scores = (un_emb * n_emb).sum(dim=1) # [batch_size]
