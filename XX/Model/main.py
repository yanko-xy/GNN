...
def use_swanlab(args):
    swanlab.login(api_key="8d1bTQsUZvD4Ki4kPvInV", save=True)
    swanlab.init(
        # 设置项目
        project="KACL",
        # 跟踪超参数与实验元数据
        config=args,
        experiment_name="KACL_Specific_Item_Att",
        description="使用具体物品的注意力机制",
    )
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    """
    weight_size = eval(args.layer_size) # 结果: [64, 32, 16]
    num_layers = len(weight_size) - 2 # 1
    heads = [args.heads] * num_layers + [1] # [1, 1]
    print(f"用户个数：{config['n_users']}\n实体个数: {config['n_entities']}\n知识图谱维度: {args.kge_size}\n关系边个数: {config['n_relations']}" )

    
    model = myGAT(args, config['n_entities'], config['n_relations'] + 1, weight_size[-2], weight_size[-1], num_layers, heads, F.elu, 0.1, 0., 0.01, False, pretrain=pretrain_data, device=device).to(device)

    adjM = data_generator.lap_list

    print(f"边个数为: {len(adjM.nonzero()[0])}" )
    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        sub_cf_g = dgl.remove_self_loop(sub_cf_g)
        sub_cf_g = dgl.add_self_loop(sub_cf_g)
        sub_cf_g = sub_cf_g.to(device)
        ...
        *********************************************************
        ...
        """
        for idx in tqdm(range(n_batch), total=n_batch, desc='Train cf') :
            model.train()
            btime= time() 
        ...
        for idx in tqdm(range(n_kg_batch), total=n_kg_batch, desc='Train kg'):
            model.train()
        ...
        for idx in tqdm(range(n_cl_batch), total=n_cl_batch, desc='Train cl') :
            model.train()
        ...
        *********************************************************
        ...
        if args.verbose > 0:
            ...
            swanlab.log({
                ...
                "recall@10": ret['recall'][0],
                "precision@10": ret['precision'][0],
                "hit@10": ret['hit_ratio'][0],
                "ndcg@10": ret['ndcg'][0],
                "recall@20": ret['recall'][1],
                "precision@20": ret['precision'][1],
                "hit@20": ret['hit_ratio'][1],
                "ndcg@20": ret['ndcg'][1],
            })
