...
class Data(object):
    def __init__(self, args, path):
        ...
        self.n_train, self.n_test = 0, 0 # 训练集数量 测试集数量
        self.n_users, self.n_items = 0, 0 # 用户数量 物品数量
        ...
        self.n_relations, self.n_entities, self.n_triples = 0, 0, 0 # 关系边数量 实体数量 三元组数量
    ...
    def _statistic_ratings(self):
        self.n_users = max(max(self.train_data[:, 0]), max(self.test_data[:, 0])) + 1 # 用户数量                                                  
        self.n_items = max(max(self.train_data[:, 1]), max(self.test_data[:, 1])) + 1 # 物品数量
        self.n_train = len(self.train_data) # 训练数据量
        self.n_test = len(self.test_data) # 测试数据量
